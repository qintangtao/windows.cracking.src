...
void encrypt_file(PIMAGE_NT_HEADERS nt_headers, pfile_info target_file, const char *included_section_name) {
    PIMAGE_SECTION_HEADER current_section = IMAGE_FIRST_SECTION(nt_headers);
    const char *included_sections[] = {".data", ".text", included_section_name};
    for(int i = 0; i < nt_headers->FileHeader.NumberOfSections; ++i) {
        int included = 0;
        for(int j = 0; j < sizeof(included_sections)/sizeof(included_sections[0]); ++j)
            included |= (strcmp(included_sections[j], (char *)current_section->Name) == 0);
        if(included == 1) {
